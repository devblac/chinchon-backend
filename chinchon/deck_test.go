...
func TestDeadwoodPoints(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			melds:          []*Meld{},
			expectedPoints: 6, // 1+2+3
		},
		{
			name: "hand with set meld",
			hand: []Card{
				{Suit: ESPADA, Number: 1},
				{Suit: ESPADA, Number: 2},
				{Suit: ESPADA, Number: 3},
			...
			expectedPoints: 10, // 2+3+5, the 1s are in meld
		},
		{
			name: "hand with run meld",
			hand: []Card{
				{Suit: ESPADA, Number: 1},
				{Suit: ESPADA, Number: 2},
				{Suit: ESPADA, Number: 3},
			...
			expectedPoints: 14, // 4+10, the 1,2,3 are in meld
		},
		{
			name: "hand with high cards",
			hand: []Card{
			...
			melds:          []*Meld{},
			expectedPoints: 30, // 10+10+10
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := calculateDeadwoodPoints(tt.hand, tt.melds)
